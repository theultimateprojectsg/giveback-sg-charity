            ...
            {loading ? <div style={s.empty}>Loading...</div> : filteredDonations.length === 0 && activeDonationFilterCount === 0 ? (
              ...
              <div>
                {paginatedDonations.map(d => {
                  ...
                  return (
                  <div key={d.id} style={{ display: 'flex', gap: 8, padding: '12px 16px 12px 10px', borderBottom: `1px solid ${C.ivoryDark}`, cursor: 'pointer', background: isRefunded ? C.dangerBg : 'transparent' }} onClick={() => { setSelectedDonation(d); setQuickEmailInput(''); setQuickNricInput('') }}>
                  ...
                  {(() => {
                    ...
                    return myRefunds119.length > 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {myRefunds119.map(r => (
                          <div key={r.id} style={{ fontSize: 12, background: C.dangerBg, border: `1px solid ${C.dangerBorder}`, borderRadius: 6, padding: '8px 10px', color: C.red, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
