  ...
  return (
    <div className="pitch-page">
      <style>{`
        ...
        .pitch-nav-link{color:rgba(255,255,255,0.6);font-size:13px;text-decoration:none}
        ...
        .pitch-glow-1{position:absolute;top:-200px;left:50%;transform:translateX(-50%);width:900px;height:600px;border-radius:50%;background:radial-gradient(ellipse,rgba(27,67,50,0.6) 0%,transparent 65%);pointer-events:none}
        .pitch-glow-2{position:absolute;bottom:-100px;right:-100px;width:500px;height:500px;border-radius:50%;background:radial-gradient(ellipse,rgba(212,160,23,0.07) 0%,transparent 65%);pointer-events:none}
        ...
        .pitch-hero-title em{font-style:italic;color:var(--gold-light)}
        ...
        .pitch-hero-actions{display:flex;flex-wrap:wrap;gap:12px;margin-top:32px;animation:pitchFadeUp .7s .3s ease both}
        .pitch-btn-primary{background:var(--gold);color:var(--forest-deep);padding:15px 32px;border-radius:100px;font-size:14px;font-weight:700;text-decoration:none;box-shadow:0 8px 32px rgba(212,160,23,0.3);transition:transform .2s,box-shadow .2s;display:inline-block;cursor:pointer;border:none}
        .pitch-btn-primary:hover{transform:translateY(-3px);box-shadow:0 14px 40px rgba(212,160,23,0.45)}
        ...
        .pitch-btn-ghost:hover{background:rgba(255,255,255,0.12);color:white}
        .pitch-hero-note{font-size:12px;color:rgba(255,255,255,0.45);margin-top:18px;animation:pitchFadeUp .7s .4s ease both}
        .pitch-trust-bar{background:var(--border);border-top:1px solid var(--border);border-bottom:1px solid var(--border);padding:18px 32px;display:flex;align-items:center;justify-content:center;gap:28px;flex-wrap:wrap;background:#F0EBE1}
        ...
        .pitch-step-num{width:52px;height:52px;border-radius:50%;background:var(--forest);color:white;font-family:Georgia,serif;font-size:19px;font-weight:700;display:flex;align-items:center;justify-content:center;margin:0 auto 16px}
        .pitch-input{width:100%;padding:12px 14px;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.25);border-radius:10px;color:white;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box}
      ...
      <section className="pitch-hero">
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, textAlign: 'left' }}>
          ...
          <div className="pitch-hero-actions">
            <a href="#contact" className="pitch-btn-primary">Request a demo →</a>
      ...
      <section style={{ background: 'var(--forest)', padding: '80px 24px' }}>
        <div className="pitch-container pitch-reveal" style={{ textAlign: 'center', maxWidth: 820, margin: '0 auto' }}>
          ...
          <div className="pitch-quote-grid">
            {PAIN_QUOTES.map((q, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.05)', borderTop: '3px solid var(--gold)', borderRadius: 12, padding: '24px 20px', textAlign: 'center' }}>
                <p className="pitch-serif" style={{ fontSize: 14.5, fontStyle: 'italic', color: 'rgba(255,255,255,0.85)', lineHeight: 1.8, margin: 0 }}>{q}</p>
              </div>
            ))}
          </div>
          <div style={{ display: 'inline-block', background: 'rgba(212,160,23,0.12)', border: '1px solid rgba(212,160,23,0.3)', borderRadius: 100, padding: '12px 28px' }}>
      ...
      <section id="features" style={{ background: 'white', padding: '80px 24px' }}>
        <div className="pitch-container">
          ...
          {/* Fake "browser window" dashboard mockup */}
          <div className="pitch-dashboard-mock pitch-reveal" style={{ marginBottom: 28, background: '#1e1e1e', borderRadius: 16, overflow: 'hidden', boxShadow: '0 40px 100px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ display: 'flex', height: 460, fontFamily: "'Segoe UI',sans-serif", fontSize: 13 }}>
              ...
              <div style={{ flex: 1, background: '#FAF7F2', overflow: 'hidden', padding: 18 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 9, marginBottom: 12 }}>
                  {[{ l: 'Total received', v: '$48,200', dark: true }, { l: 'Unique donors', v: '312' }, { l: 'Avg. donation', v: '$154' }, { l: 'Receipts pending', v: '3', warn: true }].map((s, i) => (
                    <div key={i} style={{ background: s.dark ? '#1B4332' : s.warn ? '#FDF3DC' : 'white', border: s.dark ? undefined : `1.5px solid ${s.warn ? '#E8CC7A' : '#E2D9CC'}`, borderRadius: 10, padding: 11 }}>
                      ...
                      <div style={{ fontSize: 17, fontWeight: 800, color: s.dark ? 'white' : s.warn ? '#A07010' : '#1B4332' }}>{s.v}</div>
                    </div>
                  ))}
                </div>
          ...
          <div className="pitch-feature-grid pitch-reveal">
            {FEATURE_CARDS.map((f, i) => (
              <div key={i} style={{ display: 'flex', gap: 14, padding: 22, background: 'white', border: '1.5px solid var(--border)', borderRadius: 16 }}>
      ...
      <section id="how-it-works" style={{ background: 'var(--ivory)', padding: '80px 24px' }}>
        <div className="pitch-container">
          ...
          <div className="pitch-steps-grid pitch-reveal">
            {STEPS.map(s => (
              <div key={s.n} className="pitch-step">
      ...
      <section id="contact" style={{ background: 'var(--forest)', padding: '96px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="pitch-reveal" style={{ position: 'relative', zIndex: 1, maxWidth: 520, margin: '0 auto' }}>
          ...
          {sent ? (
            <div style={{ background: 'rgba(64,145,108,0.15)', border: '1px solid #40916C', borderRadius: 16, padding: '24px 28px', fontSize: 14, color: 'white', lineHeight: 1.7, textAlign: 'left' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 24, padding: 32, textAlign: 'left' }}>
              ...
              <button onClick={submitDemoRequest} disabled={sending} className="pitch-btn-primary" style={{ width: '100%', textAlign: 'center', opacity: sending ? 0.7 : 1 }}>
